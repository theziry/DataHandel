...
f=open('MODEL2/initial/20{}_Initial_Max_LumTurn_mod4.txt'.format(str(year)),"r")
lines=f.readlines()
L_ma=[]
...
if plot==True:
    ...
    for i in range(len(res.x)):
        ...
        #x = np.where(x>1, x, 2 )
        D = b[i] * np.power(c[i] / (2 * np.exp(1)), c[i]) / (np.power(np.log(r[i]), c[i]))
        fit= lambda x: ((1 / (1 + a[i] * (x - 1)) ** 2) - ((1 + D ** 2) * np.exp(-(D ** 2))) * ((2 - (1 + D ** 2) * np.exp(-(D**2) / 2))))/(1e4)
        Li=quad(fit, 0, r[i])[0] 
        LL = Li * L0
        #Lopt.append(Li)
        Lopt.append(Li)
